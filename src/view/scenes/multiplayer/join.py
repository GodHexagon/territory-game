...
from ...sequencer.game_access import Error
from .joining_setting import JoiningSettingScene
...
class JoinScene(View, Area):
    ...
    def _hdl_get_accesskey(self, key: str):
        ...
        if isinstance(self.scene, JoiningSettingScene):
            self.scene.allowed_to_access()
        else:
            raise ValueError("パスワード入力シーン以外でアクセスキー取得が完了した。")
